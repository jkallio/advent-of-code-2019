...
class Intcode(private var pos: Int, private var base: Int) {
    private val intcode = arrayOfNulls<String>(2000)

    constructor (source: List<String>) : this(0, 0) {
        for (i in intcode.indices) {
            intcode[i] = if (i < source.count()) source[i] else "0"
        }
    }

    constructor(source: Array<String?>, pos: Int, base: Int) : this(pos, base) {
        for (i in intcode.indices) {
            intcode[i] = source.getOrElse(i) { "0" }
        }
    }
        ...
        loop@ while (pos in 0 until intcode.count()) {
            ...
            when (op) {
                ...
                3 -> { // op = 'input'
                    ...
                    writeIntValue(pos + 1, input, mode1)
                    pos += 2
                }

                4 -> { // op = 'output'
                    output = getIntValue(pos + 1, mode1)
                    pos += 2
                    break@loop
                }
    ...
    fun clone() : Intcode {
        return Intcode(intcode, pos, base)
    }
