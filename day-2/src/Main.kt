...
fun runIntCode(input: List<Int>, noun: Int, verb: Int) : Int {

    val intcode = mutableListOf<Int>()
    intcode.addAll(input)
    intcode[1] = noun
    intcode[2] = verb
    ...
    loop@ while (i < intcode.count()) {
        when (intcode[i]) {
            1,2 -> {
                val a = intcode[intcode[i+1]]
                val b = intcode[intcode[i+2]]
                val storePos = intcode[i+3]

                var opResult: Int
                if (intcode[i] == 1) {
                    opResult = a + b
                    //println("* add $a + $b = $opResult --> intcode[$storePos]")
                }
                else {
                    opResult = a * b
                    //println("* multiply $a * $b = $opResult --> intcode[$storePos]")
                }
                intcode[storePos] = opResult
                i += 4
            }
...
fun main(args: Array<String>) {
    ...
    input.split(',').forEach { intcode.add(it.toInt()) }

    val runResult = runIntCode(intcode, 12, 2)
    println("Part-1: Intcode result = $runResult")

    var noun = 0
    var verb = 0
    nounloop@ for (i in 0..99) {
        verbloop@ for (j in 0..99) {
            val result = runIntCode(intcode, i, j)
            if (result == 19690720) {
                noun = i
                verb = j
                break@nounloop
    ...
    println("Part-2: Result = 100 * $noun * $verb => $result")
}