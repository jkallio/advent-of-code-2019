 ...
 */
typealias Element = Pair<String, Long>
typealias ElementList = MutableMap<String, Long>
...
fun main(args: Array<String>) {
    args.getOrNull(0)?.let { inputFile ->
        val factory = NanoFactory(parseReactions(File(inputFile).readLines()))
        factory.create(Element("FUEL", 1))

        println("---------------------------")
        factory.supply.forEach { println(it) }
        println("---------------------------")
        //println("Total Ore consumption = ${factory.getOreConsumption()}")
