...
fun main (args: Array<String>) {
    ...
    val intcode = Intcode(input.toTypedArray())
    val gameState = GameState()

    while (!intcode.isFinished()) {
        val x = intcode.run(gameState.nextInput)
        if (x >= 0) {
            val y = 0 - intcode.run()
            val tile = intcode.run()
            val point = Point(x, y, tile)
            gameState.updateTile(point)
        }
        else {
            gameState.print()
            if (intcode.run() != 0) {
                println("*** Critical error (Expected y=0 if x=-1)")
                return
            }
            gameState.score = intcode.run()
