...
fun calculateFuel(mass : Int, includeFuelMass : Boolean) : Int {
    var fuel = kotlin.math.floor(mass.toDouble() / 3.0).toInt() - 2
    if (includeFuelMass && fuel > 0) {
        fuel += calculateFuel(fuel, true)
    }
    return kotlin.math.max(fuel, 0)
}
...
fun main(args: Array<String>) {
    ...
    var totFuel1 : Int = 0  // Part-1
    var totFuel2 : Int = 0  // Part-2
    File(args[0]).forEachLine {
        val moduleMass = it.toInt()
        totFuel1 += calculateFuel(moduleMass, false)
        totFuel2 += calculateFuel(moduleMass, true)
    }
    println("Result for Part-1 = $totFuel1")
    println("Result for Part-2 = $totFuel2")
}